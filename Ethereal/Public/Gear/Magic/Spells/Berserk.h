...
public:
	...
	virtual void ChargeBerserk();

	/** Cast Spell. */
	UFUNCTION(BlueprintCallable, Category = Casting)
	virtual void CastBerserk();

	/** Do Cast effect. */
	UFUNCTION(BlueprintCallable, Category = Casting)
	virtual void RemoveCastEffect();

	/** ATK added to the player's base stat by casting this spell. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Casting)
	float StoredATKBoost;
};
